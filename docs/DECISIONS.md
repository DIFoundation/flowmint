...
## 028 — Usage-measurement instrumentation for M6, before any real users
**Decision:** New `apps/web/src/lib/metrics-store.ts` records a raw event log (`flow_created`, `flow_escalated`, `flow_authorized`, `flow_completed`, `flow_failed`, each with wallet address and timestamp where known) at every point the existing API routes already transition a Flow's state. `GET /api/metrics` (access-key gated) and `/metrics` compute derived numbers on read: distinct users, returning users (same address across >1 calendar day), distinct signers (address on a `flow_authorized` event specifically, not just any visit), genuine transactions, and a funnel including abandoned flows (created or authorized but never reaching a terminal state).
**Reason:** M6's exit criterion requires *evidence* of real, distinct, returning users and genuine transactions. Before this, there was no way to answer any of those questions even if real people used the app today — `TRUST.md` §7 already flagged that Flow evidence doesn't aggregate across flows or persist. This closes the measurement side specifically so real usage (which only the user, not me, can generate) is actually captured when it happens.
**A bug found by building and testing it, not assuming it worked:** `next build` marked `/api/metrics` as statically prerendered (`○`) by default, which would have frozen every response at whatever empty snapshot existed at build time. Fixed with an explicit `export const dynamic = "force-dynamic"`. Caught only because the build output was actually read, not because it seemed likely to happen.
**Alternatives:** Wire a third-party analytics SDK (PostHog, Mixpanel, etc.). Rejected for now — the milestone's specific metrics (distinct signers vs. distinct users, abandonment by funnel stage) map directly onto data this app already has natively (wallet addresses, Flow state transitions); a general-purpose analytics SDK adds a dependency and a data-sharing surface for something a ~150-line module covers exactly.
**Impact:** `apps/web/src/lib/metrics-store.ts` (new), 4 existing API routes instrumented, `app/api/metrics/route.ts` + `app/metrics/page.tsx` (new), `.env.template` (`METRICS_ACCESS_KEY`). **This reuses the same in-memory, `globalThis`-anchored, non-persistent storage pattern as `agent-server.ts` (Decision 026) — meaning it has the exact same Vercel persistence gap, now with higher stakes: "measure returning users" is specifically impossible across a serverless cold start with this store.** A real datastore decision is now a precondition for real M6 evidence in production, not just a documented footnote.
